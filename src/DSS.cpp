...
void Graph::read_graph_from_dataset(char* dataset_address) {
	...
	S.alloc(N); Q.alloc(N); parent.alloc(N); vis.alloc(N); D.alloc(N);
...
void Graph::get_dense_subgraph_from_core() {
	...
	while (true) {
		Q.clear(); parent.clear(); vis.clear();
		for (int i = 0; i < S.size; i++) {
			int x = S.nodes[i];
			if (in_T(x))
				Q.push(x), parent[x] = -1, vis[x] = true;
		}
		int tar = -1;
		while (!Q.empty()) {
			int x = Q.pop();
			for (int j = 0; j < undeg[x]; j++) {
				...
				if (vis[y]) continue;
				if (in_S(y)) {
					parent[y] = adj[x][j], tar = y;
					break;
				}
				vis[y] = true; parent[y] = adj[x][j];
				Q.push(y);
			}
			if (tar != -1)
				break;
		...
		if (tar != -1) {
			while (parent[tar] != -1) {
				indeg[tar]++, indeg[e[parent[tar]].to]--;
				swap(e[parent[tar]].to, tar);
			}
		}
		else
			break;
